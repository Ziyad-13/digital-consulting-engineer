...
def _render_material_gate(project_id: int, phase_number: int, pdef: dict) -> None:
    ...
    materials_state = db.get_materials(project_id, phase_number)
    boq_items = {item["item_name"]: item for item in db.get_boq_items(project_id)}

    for mat in pdef["materials"]:
        _render_material_row(project_id, phase_number, mat, materials_state, boq_items)


def _render_material_row(project_id: int, phase_number: int, mat: dict, materials_state: dict, boq_items: dict) -> None:
    name = t(mat["name_key"])
