...
def _render_material_gate(project_id: int, phase_number: int, pdef: dict) -> None:
    ...
    all_boqs = {b["item_name"]: b for b in db.get_boq_items(project_id)}
    materials_state = db.get_materials(project_id, phase_number)

    for mat in pdef["materials"]:
        _render_material_row(project_id, phase_number, mat, all_boqs, materials_state)


def _render_material_row(project_id: int, phase_number: int, mat: dict, all_boqs: dict, materials_state: dict) -> None:
    name = t(mat["name_key"])
    boq_row = all_boqs.get(mat["boq_match_name"])
