...
def _render_checklist_item(
    ...
    with st.expander(f"{badge}  {label}{suffix}",
        ...
        if pass_disabled:
            helps = []
            if material_blocked:
                helps.append(t("alert.material_required"))
            if image_blocked:
                helps.append(t("alert.image_required"))
            pass_help = " ".join(helps)

        if col_pass.button(
            t("btn.mark_pass"),
            key=f"pass_{phase_number}_{key}",
            disabled=pass_disabled,
            help=pass_help if pass_disabled else None,
            type="primary",
        ):
            db.upsert_checklist_item(
        ...
        if col_rework.button(
            ...
            help=t("alert.fail_explain") if rework_disabled else None,
        ):
