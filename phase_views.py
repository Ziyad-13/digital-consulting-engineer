...
def _render_material_gate(project_id: int, phase_number: int, pdef: dict) -> None:
    ...
    # ⚡ Bolt Optimization: Fix N+1 query bottleneck by fetching data once outside the render loop
    materials_state = db.get_materials(project_id, phase_number)
    boq_items = {row["item_name"]: row for row in db.get_boq_items(project_id)}

    for mat in pdef["materials"]:
        _render_material_row(project_id, phase_number, mat, materials_state, boq_items)


def _render_material_row(
    project_id: int, phase_number: int, mat: dict,
    materials_state: dict[str, dict], boq_items: dict[str, dict]
) -> None:
    name = t(mat["name_key"])
    boq_row = boq_items.get(mat["boq_match_name"])
